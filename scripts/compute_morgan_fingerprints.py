...
def parse_args() -> argparse.Namespace:
  ...
  p.add_argument(
    "--input", default="data/ersilia_reference_library_v0.csv", help="CSV with the reference library."
  )
  p.add_argument(
    "--output",
  ...
  p.add_argument("--radius", type=int, default=3, help="Morgan radius (eos5axz uses 3).")
  p.add_argument(
    "--nbits", type=int, default=2048, help="Fingerprint size / folded bins (eos5axz uses 2048)."
  )
  p.add_argument("--njobs", type=int, default=os.cpu_count() or 1, help="Parallel worker processes.")
  p.add_argument("--batch-size", type=int, default=8192, help="Rows written per HDF5 slice.")
  p.add_argument("--limit", type=int, default=None, help="Cap on the number of compounds (development).")
  return p.parse_args()
...
def main() -> None:
  ...
  logger.info(f"Morgan count fingerprints: radius={args.radius} nbits={args.nbits} (counts clipped at 255)")
  ...
  with h5py.File(out_path, "w") as h5:
    ...
    with logger.stage("Computing Morgan count fingerprints"):
      ...
      start = 0
      with (
        Pool(processes=args.njobs, initializer=_init, initargs=(args.radius, args.nbits)) as pool,
  ...
  logger.info(f"data matrix: shape=({n}, {args.nbits}), dtype=uint8, size={nbytes / 1e9:.2f} GB")
