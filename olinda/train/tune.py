...
def _load_subset(run_dir: Path, max_rows: int | None, seed: int):
    ...
    matrix = ReferenceMatrix.load(
        OLINDA_HOME / MORGAN_FINGERPRINTS_FILENAME, limit=runlib.row_limit(manifest)
    )
...
def run_tuning(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "optuna is required — install the train extra:  pip install -e '.[train]'"
        ) from e
    ...
    echo(
        f"optimizing: learning_rate [{lr_lo}–{lr_hi}] · min_split_gain [{g_lo:g}–{g_hi:g}]",
        "run",
    )
    echo(
    ...
    def objective_fn(trial):
        ...
        best_score, best_it, _m = be.train_trial(
            dtrain, dval, native, tune_rounds, early_stopping, _on_iter
        )
    ...
    def _after_trial(study, trial):
        ...
        if patience and progress["stale"] >= patience:
            echo(
                f"converged — no improvement in {patience} trials; stopping early",
                "info",
            )
            study.stop()
    ...
    study = optuna.create_study(
        direction=direction, sampler=sampler, pruner=optuna.pruners.HyperbandPruner()
    )
    ...
    if not completed:
        echo(
            "no trials completed — nothing to report (try a larger --time-budget or fewer rounds)",
            "error",
        )
        return {}
    ...
    tuned = {
        k: (round(v, 4) if isinstance(v, float) else v) for k, v in best.params.items()
    }
    ...
    rows = [
        (
            f"Best {metric}",
            f"[bold]{best.value:.5f}[/]  [dim]· at ~{best_iter} rounds (of ≤{tune_rounds})[/]",
        ),
    ...
    echo(
        "Paste-ready CANONICAL_DEFAULTS (backend + objective are resolved automatically at train time):",
        "info",
    )
