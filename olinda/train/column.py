...
def train_one_column(
    ...
    reweight_info["used_in"] = {
        "training": weighted,
        "evaluation": weighted,
        "early_stopping": weighted,
    }
    ...
    featurizer = MorganCountFeaturizer(
        radius=int(manifest["features"].get("radius", 3)), fp_size=x_dim
    )
    student = StudentModel(
        ...
        metadata={
            ...
            "hyperparams": {
                "source": "tuned" if tuned_params else "defaults",
                "tuned": tuned_params or None,
            },
        },
    )
    ...
    if len(yval) >= 4 and np.isfinite(yval).any():
        IsotonicCalibrator().fit(
            raw=pval, target=np.asarray(yval, dtype=np.float64)
        ).save(col_dir / "calibrator.json")

    save_true_vs_pred(
        yval,
        pval,
        col_dir / "val_true_pred.png",
        title=f"{col['name']}  (R²={metrics['r2']:.3f})",
    )
    if not quiet:
        echo(
...
def resolve_tuned_params(model_dir: Path, col_dir: Path, backend_name: str) -> dict:
    ...
    path = next(
        (
            p
            for p in (col_dir / runlib.PARAMS_NAME, model_dir / runlib.PARAMS_NAME)
            if p.exists()
        ),
        None,
    )
    if path is None:
        echo(
            "no best_params.json — using built-in defaults (`olinda tune -m` to tune)",
            "info",
        )
        return {}
    ...
    if tag and tag != backend_name:
        echo(
            f"best_params.json was tuned on {tag}; its canonical params still apply to {backend_name}",
            "warning",
        )
    unknown = [k for k in tuned if k not in CANONICAL_DEFAULTS]
    if unknown:
        echo(
            f"ignoring unrecognized best_params.json keys {unknown} — re-run `olinda tune`",
            "warning",
        )
        for k in unknown:
            tuned.pop(k)
