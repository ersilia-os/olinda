...
def select_backend() -> tuple[str, str, str]:
  ...
  if override in ("xgboost", "lightgbm"):
    ...
    raise ValueError(f"OLINDA_BACKEND must be auto|xgboost|lightgbm, got {override!r}")
  if name == "lightgbm":
    try:
      import lightgbm  # noqa: F401
    except ImportError:
      echo("lightgbm not installed — falling back to XGBoost on CPU (pip install lightgbm)", "warning")
...
class XGBoostBackend:
  ...
  def train(
    ...
    best_score = float(val_hist[best_it]) if best_it < len(val_hist) else float("nan")
    return TrainResult(booster, best_it, best_score, metric, booster.num_boosted_rounds())

  # -- tune path (in-RAM subset, per-round pruning hook) --
  def dataset(self, X, y, weight, max_bin, reference=None):
    import xgboost as xgb

    return xgb.QuantileDMatrix(X, label=y, weight=weight, max_bin=max_bin, ref=reference)

  def train_trial(self, dtrain, dval, native_params, num_boost_round, early_stopping, on_iteration):
    import xgboost as xgb
...
class LightGBMBackend:
  ...
  def train(
    ...
    st = {"best": float("inf"), "best_it": 0, "r2": float("nan"), "rho": float("nan")}
    ...
    with live_status() as update:
      ...
      def _cb(env):
        it = env.iteration
        va = next(
          (float(v) for n, m, v, _ in env.evaluation_result_list if n == "val" and m == metric), float("nan")
        )
        if va < st["best"]:
          st["best"], st["best_it"] = va, it
        if it % 100 == 0 and xval is not None:  # R²/ρ need a val prediction pass — throttle it
          _, st["r2"], st["rho"] = _val_stats(yval, np.asarray(env.model.predict(xval), dtype=np.float64))
        if it % 25 == 0:
          update(
  ...
  def train_trial(self, dtrain, dval, native_params, num_boost_round, early_stopping, on_iteration):
    ...
    return float(booster.best_score["val"][metric]), int(booster.best_iteration), metric
  ...
  def save(self, model, model_dir):
    ...
    model.save_model(str(Path(model_dir) / self.model_file), num_iteration=model.best_iteration)
