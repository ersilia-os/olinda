...
def tanimoto_nn(
    query_bits: np.ndarray, hard_bits: np.ndarray = None, *, prepared=None
) -> np.ndarray:
    ...
    for start in range(0, n, _NN_BLOCK):
        ...
        np.divide(
            inter, union, out=inter
        )  # in place: the intersection block is dead after this
...
class _MLP:
    ...
    def __init__(
        ...
        *,
        ...
        for fan_in, fan_out in zip(sizes[:-1], sizes[1:]):
            ...
            self.weights.append(
                rng.uniform(-limit, limit, (fan_in, fan_out)).astype(np.float32)
            )
    ...
    def partial_fit(self, x: np.ndarray, y: np.ndarray) -> None:
        ...
        delta = (acts[-1] - y.reshape(-1, 1).astype(np.float32)) * np.float32(
            2.0 / len(x)
        )
        self._steps += 1
        ...
        scale = np.float32(
            self.l2 * 2.0 / len(x)
        )  # delta already carries 2/batch; keep the penalty in step
        for i in range(depth - 1, -1, -1):
            grad_w = acts[i].T @ delta
            if self.l2:
                grad_w += (
                    scale * self.weights[i]
                )  # biases stay unpenalised, as they should
    ...
    def to_onnx(
        ...
        *,
        ...
        for i, (w, b) in enumerate(zip(self.weights, self.biases)):
            ...
            nodes.append(
                helper.make_node(
                    "Add", [f"t_z{i}", f"t_b{i}"], [output_name if last else f"t_a{i}"]
                )
            )
            if not last:
        ...
        graph = helper.make_graph(
            ...
            [
                helper.make_tensor_value_info(
                    input_name, TensorProto.FLOAT, ["B", int(n_features)]
                )
            ],
        ...
        model = helper.make_model(
            graph, opset_imports=[helper.make_opsetid("", _T_OPSET)]
        )
...
def ramp(
    similarity, *, a_max: float = A_CEILING, sim_lo: float = T_LO, sim_hi: float = T_HI
):
        ...
        for epoch in range(T_MAX_EPOCHS):
            for bits, target in batches(rng):
                ...
                for start in range(0, len(x), T_SGD_BATCH):
                    net.partial_fit(
                        x[start : start + T_SGD_BATCH], y[start : start + T_SGD_BATCH]
                    )
    ...
    def _run(self):
        ...
        if self._session is None:
            ...
            self._session = ort.InferenceSession(
                self.onnx_bytes, options, providers=["CPUExecutionProvider"]
            )
