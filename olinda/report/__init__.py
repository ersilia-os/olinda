...
def require_report_extra() -> None:
    ...
    missing = []
    for module, why in (
        ("onnx", "read the model graph"),
        ("stylia", "draw the figures"),
    ):
        try:
            __import__(module)
        except ImportError:
...
def _match_columns(value_cols, tasks) -> tuple[dict, list]:
    ...
    for col in value_cols:
        ...
        for source, task in pair.items():
            if task in matched:
                conflicts.append(
                    f"{matched[task]!r} and {source!r} both name task {task!r}"
                )
            matched[task] = source
    if conflicts:
...
def _read_labels(
    ...
    try:
        smiles_arr, values = resolve_smiles_frame(
            frame, smiles_column=smiles_column, label_columns=label_columns
        )
    ...
    return (
        smiles_arr.tolist(),
        {task: values[src].to_numpy() for task, src in matched.items()},
        ignored,
    )
...
def _hard_head_warning(kind: str, artifact, n_rows: int) -> list[str]:
    ...
    for column in artifact.metadata.get("columns", []):
        hard = next(
            (h for h in (column.get("heads") or []) if h.get("role") == "hard"), None
        )
...
def _predict(artifact, smiles, echo=None) -> "dict[str, np.ndarray]":
    ...
    with warnings.catch_warnings():
        ...
        )  # unparseable SMILES are counted, not warned twice
...
def validate_model(
    ...
    *,
    ...
    report: dict = {
        ...
        "model": {
            **artifact.describe(),
            "backend": (artifact.metadata.get("run") or {}).get("backend"),
        },
        "notes": [],
    ...
    for kind, source in (("soft", soft_labels), ("hard", hard_labels)):
        ...
        echo(
            f"{kind} labels · [bold]{len(smiles):,}[/] compounds · {', '.join(values)}",
            "run",
        )
        if ignored:
            report["notes"].append(
        ...
        for task, truth in values.items():
            ...
            if len(y) == 0:
                report["notes"].append(
                    f"no comparable rows for {task} in the {kind} labels — skipped"
                )
                continue
            ...
            if kind == "soft":
                ...
                for name, draw in (
                    (
                        "correlation",
                        lambda ax, st, y=y, p=p, m=m, t=task: plots.correlation(
                            ax, st, y, p, metrics=m, task=t
                        ),
                    ),
                    (
                        "residuals",
                        lambda ax, st, y=y, p=p: plots.residual_hist(ax, st, y, p),
                    ),
                    (
                        "residual_structure",
                        lambda ax, st, y=y, p=p: plots.residuals_vs_pred(ax, st, y, p),
                    ),
                    (
                        "calibration",
                        lambda ax, st, y=y, p=p: plots.calibration_bins(ax, st, y, p),
                    ),
                    (
                        "residual_qq",
                        lambda ax, st, y=y, p=p: plots.qq_residuals(ax, st, y, p),
                    ),
                    (
                        "score_distributions",
                        lambda ax, st, y=y, p=p: plots.score_distributions(
                            ax, st, y, p
                        ),
                    ),
                ):
                    fig = plots.render(
                        draw, out_dir, f"{task}_{name}", cells=plots.CELLS.get(name)
                    )
                    if fig:
                ...
                if not set(binary.tolist()) <= {0.0, 1.0}:
                    report["notes"].append(
                        f"{task}: hard labels are not 0/1, skipping the ranking metrics"
                    )
                    continue
                ...
                for name, draw in (
                    (
                        "roc",
                        lambda ax, st, y=y, p=p, m=m: plots.roc(
                            ax, st, y, p, metrics=m
                        ),
                    ),
                    (
                        "precision_recall",
                        lambda ax, st, y=y, p=p, m=m: plots.precision_recall(
                            ax, st, y, p, metrics=m
                        ),
                    ),
                    (
                        "enrichment",
                        lambda ax, st, y=y, p=p: plots.enrichment(ax, st, y, p),
                    ),
                    (
                        "score_by_class",
                        lambda ax, st, y=y, p=p: plots.score_by_class(ax, st, y, p),
                    ),
                ):
                    fig = plots.render(
                        draw, out_dir, f"{task}_{name}", cells=plots.CELLS.get(name)
    ...
    for task, info in internals.items():
        ...
        for stage, curve, label, xlabel, role in (
            ...
            fig = plots.render(
                lambda ax, st, c=curve, t=label, x=xlabel, r=role: (
                    plots.calibration_map(ax, st, c, title=t, xlabel=x, role=r)
                ),
                out_dir,
                f"{task}_{stage}",
            )
