...
def detect_imbalance_from_y(
    ...
    imbalance_ratio = (
        float(non_empty.max() / non_empty.min()) if len(non_empty) > 1 else 1.0
    )
...
def density_weights_from_y(
    ...
    typical = float(
        dens_at_y.mean()
    )  # density a typical sample sees → this density gets weight 1
    with np.errstate(divide="ignore", invalid="ignore"):
...
def choose_weighting_strategy(
    ...
    is_imbalanced, cv, details = detect_imbalance_from_y(
        y_all, n_bins=n_bins, cv_threshold=cv_threshold
    )

    info = {"cv": round(float(cv), 4), "n": n, "n_unique": n_unique}
    if not is_imbalanced:
        return "none", {
            "reason": f"balanced target (cv={cv:.2f} ≤ {cv_threshold})",
            **info,
        }
    if n < min_n_for_kde or n_unique < min_unique_for_kde:
        return "bins", {
            "reason": f"imbalanced but discrete/small (n={n}, unique={n_unique})",
            **info,
        }
    return "kde", {
        "reason": f"imbalanced continuous (cv={cv:.2f}, unique={n_unique})",
        **info,
    }
...
def resolve_regression_weights(
    ...
    *,
    ...
    if mode in ("off", "none"):
        ...
        if (
            strategy == "none"
        ):  # forced on: balanced target, but pick a strategy by continuity anyway
            n_rows, n_uniq = info.get("n", len(y_all)), info.get("n_unique", 0)
            strategy = "kde" if (n_rows >= 1000 and n_uniq >= 20) else "bins"
            info = {
                **info,
                "reason": f"forced on despite balanced target (cv={info.get('cv')})",
            }
    ...
    if strategy == "bins":
        edges, weights = regression_weights_from_y(
            y_all, n_bins=n_bins, max_weight=max_weight
        )
    else:
        edges, weights = density_weights_from_y(
            y_all, alpha=alpha, max_weight=max_weight, n_grid=n_grid
        )
    info = {
        "strategy": strategy,
