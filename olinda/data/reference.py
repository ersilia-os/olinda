...
def smiles_column_index(df, name: str | None = None) -> int | None:
  ...
  if name is not None:
    ...
    if wanted not in lower:
      raise ValueError(f"no column named '{name}'; available={[str(c) for c in df.columns]}")
...
def select_value_columns(values, columns=None, *, what: str = "value"):
  ...
  if missing:
    raise ValueError(f"{what} column(s) not found: {missing}; available={list(values.columns)}")
...
def resolve_smiles_frame(df, *, smiles_column: str | None = None, label_columns=None):
  ...
  if label_columns is not None:
    ...
    return df.iloc[:, si].astype(str).to_numpy(), select_value_columns(candidates.copy(), label_columns)
...
def load_reference_calcs_frame(
  path: str | Path, descriptors_h5: str | Path, columns=None, *, smiles_column: str | None = None
) -> tuple[list[str], dict]:
