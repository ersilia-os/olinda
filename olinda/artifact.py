...
def _as_smiles_list(smiles) -> list[str]:
    ...
    if isinstance(smiles, (dict, set, frozenset)):
        raise TypeError(
            f"expected an ordered sequence of SMILES, got {type(smiles).__name__} — order matters"
        )
    try:
        return [str(s) for s in smiles]
    except TypeError as exc:
        raise TypeError(
            f"expected a sequence of SMILES, got {type(smiles).__name__}"
        ) from exc
...
class _Progress:
    ...
    def __init__(
        self, total: int, label: str = "olinda", enabled: bool | None = None
    ) -> None:
    ...
    def advance(self, n: int) -> None:
        ...
        sys.stderr.write(
            f"\r  {self.label} ▕{bar}▏ {frac:>4.0%} · {self._done:,}/{self.total:,}"
        )
        sys.stderr.flush()
...
class OlindaArtifact:
    ...
    def __init__(self, path: str | Path, *, check_rdkit: bool = True) -> None:
        ...
        if self._featurizer is None:
            raise ValueError(
                f"{path} records an unsupported featurizer: {self.metadata.get('featurizer_class')}"
            )
        ...
        if not self._columns:
            raise ValueError(
                f"{path} records no columns — rebuild it with `olinda export`."
            )
        ...
        missing = [
            c["output"] for c in self._columns if c["output"] not in self._output_names
        ]
        if missing:
            raise ValueError(
    ...
    def _column(self, task: str) -> dict:
        for c in self._columns:
            if c["name"] == task:
                return c
        raise KeyError(
            f"{task!r} is not a task of this model; it predicts {self.columns}"
        )
    ...
    def featurize(self, smiles) -> np.ndarray:
        ...
        return np.asarray(
            self._featurizer.transform([str(s) for s in smiles]), dtype=np.float32
        )
    ...
    def run(self, smiles, batch_size: int = _BATCH, progress: bool | None = None):
        ...
        with _Progress(len(smiles), enabled=progress) as bar:
            for start in range(0, len(smiles), batch_size):
                ...
                block = {
                    n: np.asarray(v, dtype=np.float64).ravel()
