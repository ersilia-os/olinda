...
def new_manifest(
    *, soft_labels, hard_labels, reference, features, val_frac, seed, limit
) -> dict:
...
def add_column(
    manifest: dict, *, name: str, y, train_idx, val_idx, hard: dict | None = None
) -> dict:
...
def find_column(manifest: dict, name_or_id: str) -> dict:
    for col in manifest["columns"]:
        if name_or_id in (col["name"], col["id"]):
            return col
    raise KeyError(
        f"no column {name_or_id!r} in this run; have {[c['name'] for c in manifest['columns']]}"
    )
