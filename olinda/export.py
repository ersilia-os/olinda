...
def isotonic_to_onnx(
    cal, out_path: Path, *, in_name: str = "input", out_name: str = "output"
) -> dict:
    ...
    grid = np.linspace(
        float(cal._x.min()) - 0.1 * span, float(cal._x.max()) + 0.1 * span, 2000
    )
    raw = (cal._sign * grid).astype(np.float32)
    sess = ort.InferenceSession(
        out_path.read_bytes(), providers=["CPUExecutionProvider"]
    )
    got = sess.run(None, {in_name: raw})[0]
    n_knots = int(
        len(_thin_isotonic(cal._x, cal._y, _ISOTONIC_TOL, _ISOTONIC_MAX_KNOTS)[0])
    )
    return {
        "knots": n_knots,
        "max_abs_diff": float(np.max(np.abs(got - cal.transform(raw)))),
    }
...
def tanimoto_to_onnx(
    clf, n_features: int, out_path: Path, *, in_name: str = "input"
) -> dict:
