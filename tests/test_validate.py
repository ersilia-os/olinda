...
def fitted(tmp_path_factory):
    ...
    pd.DataFrame({"smiles": library, TASK: scaled(library)}).to_csv(
    ...
    held = _HELD_OUT * 5
    pd.DataFrame({"smiles": held, TASK: scaled(held)}).to_csv(
        tmp_path / "heldout_soft.csv", index=False
    )
    pd.DataFrame({"smiles": held, "assay": binary(held)}).to_csv(
...
def test_reported_metrics_are_the_ones_olinda_computes(fitted):
    ...
    expected = regression_metrics(
        soft[TASK].to_numpy(), artifact.run(soft["smiles"].tolist())[TASK].to_numpy()
    )
    assert metrics["soft"]["metrics"][TASK]["r2"] == pytest.approx(expected["r2"])
    assert metrics["soft"]["metrics"][TASK]["spearman"] == pytest.approx(
        expected["spearman"]
    )
...
def test_column_matching_is_forgiving_but_never_silent(fitted):
    ...
    pd.DataFrame({"smiles": _HELD_OUT, "unrelated": 1.0}).to_csv(
        tmp_path / "nope.csv", index=False
    )
    result = run_cli(
        [
            "validate",
...
def test_in_sample_scoring_is_flagged_rather_than_reported_as_performance(fitted):
    ...
    assert any("reference library" in note for note in library["notes"]), library[
        "notes"
    ]
    ...
    held_out = _validate(
        model, tmp_path / "heldout", "-h", tmp_path / "heldout_hard.csv"
    )
...
def test_a_model_can_describe_itself_with_no_labels_at_all(fitted):
    ...
    assert metrics["figures"]["internals"], (
        "the calibration curves should still be drawn"
    )
...
def test_every_figure_the_report_draws_has_a_title_and_caption():
    ...
    for key, (title, text) in plots.FIGURES.items():
        ...
        assert plots.caption(f"some_task_name_{key}") == (title, text), (
...
def test_the_page_and_the_figures_take_their_colours_from_one_table():
    ...
    for role, name in style.ROLES.items():
        ...
        assert style.hexcol(role).lower() == from_plot.lower(), (
            f"{role} disagrees with stylia"
        )
        assert style._PAPER_FALLBACK[name].lower() == from_plot.lower(), (
            f"{name} fallback is stale"
        )
