...
@pytest.mark.parametrize(
  ...
  [
    ...
    (lambda meta: meta["columns"][0].update(output="no_such_output"), "does not produce"),
  ],
  ids=["no metadata", "unknown schema", "output the graph lacks"],
)
def test_a_file_that_lies_about_itself_is_refused_at_load(tmp_path, artifact, mutate, message):
...
def test_inputs_that_would_mislead_are_refused(artifact):
  ...
  with pytest.raises(TypeError, match="sequence of SMILES"):
    model.run(42)
  for bad in (0, -1):  # zero used to reach range() and fail with 'arg 3 must not be zero'
