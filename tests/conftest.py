...
def build_artifact(
    run_dir: Path, names=("assay_probability",), *, backend: str = "xgboost"
) -> Path:
    ...
    try:
        ...
        for i, col_name in enumerate(names):
            ...
            entry = runlib.add_column(
                manifest, name=col_name, y=y, train_idx=idx, val_idx=idx
            )
            targets[entry["id"]] = y
        ...
        runlib.write_splits(
            run_dir,
            {cid: (np.arange(len(smiles)), np.arange(len(smiles))) for cid in targets},
        )
    ...
    for name in names:
        if name not in have:
            model.graph.output.append(
                helper.make_tensor_value_info(name, TensorProto.DOUBLE, ["B"])
            )
