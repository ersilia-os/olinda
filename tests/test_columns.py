...
# ── shared matrix + indices trains the same model as per-column files ─────────


@pytest.mark.parametrize("backend", ["xgboost", "lightgbm"])
def test_indexed_training_matches_per_column_files(backend, tmp_path, monkeypatch):
  """The shared-library path must be a pure storage change, not a modelling change."""
  pytest.importorskip(backend)
  import h5py

  from olinda.data.dataset import resolve_regression_weights
  ...
  monkeypatch.setenv("OLINDA_BACKEND", backend)
  rng = np.random.default_rng(0)
  x = rng.integers(0, 5, size=(3000, 64), dtype=np.uint8)
  y = (x[:, :20].sum(1) / 100.0).astype(np.float32)
  train_idx, val_idx, _ = split_reference_to_indices(y, val_frac=0.2, seed=42)

  name, device, _ = select_backend()
  be = get_backend(name, device)
  edges, weights, _ = resolve_regression_weights(y[train_idx], mode="none")
  canonical = dict(CANONICAL_DEFAULTS)
  params = {**be.translate(canonical), **be.objective_params()}
  fit = dict(num_boost_round=30, early_stopping=10, train_weighted=False)

  # today's path: a materialised per-column split on disk
  for stem, idx in (("train", train_idx), ("val", val_idx)):
    with h5py.File(tmp_path / f"{stem}.h5", "w") as f:
  ...
  # multi-column path: one resident matrix addressed by index
  from_index = be.train(
    *be.build_train_val_indexed(
      ReferenceMatrix(x), y, train_idx, val_idx, canonical["max_bin"], edges, weights
    ),
    params,
    **fit,
  )

  xval = x[val_idx].astype(np.float32)
  assert np.allclose(be.predict(from_files.model, xval), be.predict(from_index.model, xval), atol=1e-6)
